...
public class App {
	public static void main(String[] args) {
		...
		int de = 1, c = 0, tries = 1, dif = 1,
				// Tolerance arrays for positions of ball and hole
				// Example size=2 means ball-position-arrays will be {-0.2,-0.1,ball
				// position,0.1,0.2}
				ballSize = 1, holeSize = 2;
		// mid position of ball or hole
		double ballM = 0, holeM = 0,
				// basic state
				angle = 0, speed = 0, g = 9.8, max = 1, min = 0, wef = 0, wmax = 0, wmin = 0;
		...
		while (run) {
			...
			if (tries == 1) {// Start a new game
				...
				for (int i = 0; i < par.length - 1; i++) {
					System.out.println(i + "-Par " + (i + 3) + ": " + FixInt.printN(par[i]) + "m-"
			...
			if (wef < 0.0) {
				System.out.println("The wind is against you,  " + FixDouble.printD(wef, de) + "m/s");
			}
			if (wef > 0.0) {
				System.out.println("The wind is towards goal, +" + FixDouble.printD(wef, de) + "m/s");
			}
			speed += wef;
			...
			if (GolfM.isHit(GolfM.getSize(FixDouble.getRounding(ballM, de), ballSize, de),
					GolfM.getSize(FixDouble.getRounding(holeM, de), holeSize, de)) || tries > (c + 2)) {
				System.out.println();
				...
				for (int i = 0; i < travel.length; i++) {
					System.out.println((i + 1) + "   " + FixDouble.printD(togoal[i], de, (int) max)
							+ "m to goal, you shot " + FixDouble.printD(travel[i], de, (int) max) + "m");
				}
				System.out.println();
				run = YesOrNo.bool();
